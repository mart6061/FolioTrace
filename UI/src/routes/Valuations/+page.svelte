<script lang="ts">
  ...
  function currencyLabel(currency: Currency) {
    return `${currency.alphabeticCode} - ${currency.name}`;
  }
  ...
  async function loadValuationHistory() {
    ...
    try {
      const historyUrl = new URL('/Valuations/History', window.location.origin);
      historyUrl.searchParams.set('valuationDateTime', toApiDateTime(data.valuationDate));
...
<svelte:head>
  <title>Valuations | FolioTrace</title>
</svelte:head>
  ...
  <div class="mx-auto grid max-w-7xl gap-5">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        ...
        <h1 class="text-2xl font-semibold text-slate-950">Valuations</h1>
      </div>
      <button
    ...
    <form class="grid gap-3 rounded-md border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-[repeat(5,minmax(0,1fr))_auto]" method="GET">
      {#if data.auditDateTime}
      ...
      <label class="grid gap-1 text-sm font-medium text-slate-700">
        Account
        <select class="h-10 rounded-md border border-slate-300 bg-white px-3 text-slate-950 shadow-sm outline-none focus:border-teal-600 focus:ring-2 focus:ring-teal-600/20" name="accountID" onchange={handleAccountChange}>
      ...
      <label class="grid gap-1 text-sm font-medium text-slate-700">
        Holding basis
        <select class="h-10 rounded-md border border-slate-300 bg-white px-3 text-slate-950 shadow-sm outline-none focus:border-teal-600 focus:ring-2 focus:ring-teal-600/20" name="holdingDateBasis">
      ...
      <label class="grid gap-1 text-sm font-medium text-slate-700">
        Price basis
        <select class="h-10 rounded-md border border-slate-300 bg-white px-3 text-slate-950 shadow-sm outline-none focus:border-teal-600 focus:ring-2 focus:ring-teal-600/20" name="instrumentPriceBasis">
      ...
      <label class="grid gap-1 text-sm font-medium text-slate-700">
        Currency
        <select class="h-10 rounded-md border border-slate-300 bg-white px-3 text-slate-950 shadow-sm outline-none focus:border-teal-600 focus:ring-2 focus:ring-teal-600/20" name="valuationCurrency">
      ...
      <button class="h-10 self-end rounded-md bg-teal-700 px-4 text-sm font-semibold text-white shadow-sm hover:bg-teal-800" type="submit">Apply</button>
    </form>
