...
public static class AccountActiveModifiedEventBuilder
{
    public static Result<AccountActiveModifiedEvent> Create(AccountActiveModifiedRequest request, Accounts? accounts = null)
    {
        if (request is null)
    ...
    public static Result<AccountActiveModifiedEvent> CreateSeed(EventID eventId, UserID userId, EventDateTime eventDateTime, AuditDateTime auditDateTime, string reason, AccountID accountID, Active active, Accounts? accounts = null)
    {
        ...
        return validationErrors.Count == 0
            ? Result<AccountActiveModifiedEvent>.Success(new AccountActiveModifiedEvent(eventId!, userId!, eventDateTime!, auditDateTime!, reason, accountID!, active))
            : Result<AccountActiveModifiedEvent>.Invalid(validationErrors);
    }
}
