...
public sealed class AggregateUpdateNotificationService : IAggregateCacheInvalidator
{
    ...
    private static IEnumerable<string> AggregateKindsFor(IAuditEventBase @event) =>
        @event switch
        {
            IAccountEvent => ["Accounts", "ProfitLoss"],
            ICountryEvent => ["Countries"],
            ICurrencyEvent => ["Currencies"],
            IFXEvent => ["FXs", "FXRates", "ProfitLoss"],
            IFXRateEvent => ["FXRates", "ProfitLoss"],
            IFoleoTraderOrderEvent => ["FoleoTraderOrders"],
            IHoldingEvent => ["Holdings", "HoldingPositions", "ProfitLoss"],
            IInstrumentEvent => ["Instruments", "InstrumentValues", "ProfitLoss"],
            IInstrumentPriceEvent => ["InstrumentValues", "ProfitLoss"],
            IInstrumentIncomeEvent => ["InstrumentValues", "ProfitLoss"],
            ITransactionEvent => ["Transactions", "HoldingPositions", "ProfitLoss"],
            ITicket => ["Tickets"],
            IUserEvent => ["Users"],
            ...
            IInputControlSettingsEvent => ["InputControlSettings", "InputPolicies"],
            IDateControlSettingsEvent => ["DateControlSettings"],
            IUserDateControlSettingsEvent => ["UserDateControlSettings", "DateControlSettings"],
