...
public sealed class ReportConfigService(IEventRepository eventRepository) : IReferenceDataService<ReportConfigs, ReportConfigServiceDiagnostics>
{
    ...
    public int Invalidate(IReportEvent @event) => InvalidateFrom(@event.EventDateTime);
    ...
    public async Task<ReportConfigs> Get(EventDateTime valuationDate)
    {
        ...
        var cacheKey = ReportConfigCacheKey.ForAllAuditHistory(valuationDate);
        var lastEventID = await eventRepository.GetLastEventIDAsync(Constants.Initialisation.ReportConfigsStreamId, valuationDate.Value);
