...
public sealed class InstrumentService(IEventRepository eventRepository) : IReferenceDataService<Instruments, InstrumentServiceDiagnostics>
{
    private readonly Lock cacheLock = new();
    private readonly Dictionary<InstrumentCacheKey, Instruments> cache = [];
    ...
    public async Task<Instruments> Get(EventDateTime valuationDate)
    {
        ...
        var cacheKey = InstrumentCacheKey.ForAllAuditHistory(valuationDate);
        var lastEventID = await eventRepository.GetLastEventIDAsync(Constants.Initialisation.InstrumentsStreamId, valuationDate.Value);
