    ...
    public static Result<TicketProposalModifiedEvent> ModifyProposal(TicketProposalRequest request, Tickets tickets) =>
        CreateResult(() =>
        {
            ...
            return messages.Count > 0
                ...
                : Result<TicketProposalModifiedEvent>.Success(new TicketProposalModifiedEvent(NewEventID(), request.UserID, request.EventDateTime, AuditDateTimeBuilder.Create(), request.Reason, request.TicketNumber, request.TargetPrice, tradeCurrency!, request.Allocations));
        });
}
