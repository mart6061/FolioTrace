    ...
    public static Result<TicketTradeFillModifiedEvent> ModifyFill(TicketTradeFillRequest request, Tickets tickets) =>
        CreateResult(() =>
        {
            ...
            return messages.Count > 0
                ...
                : Result<TicketTradeFillModifiedEvent>.Success(new TicketTradeFillModifiedEvent(NewEventID(), request.UserID, request.EventDateTime, AuditDateTimeBuilder.Create(), request.Reason, request.TicketNumber, request.FillID!.Value, request.BrokerLEI!, request.Price, request.Quantity, request.Note ?? string.Empty));
        });
}
